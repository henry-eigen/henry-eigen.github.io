...
Consider a node $$t$$, whose modeled attribute $$A_t$$ has response options $$k=1,\ldots,K$$. Let $$x=x(s_t)$$ be a multi-hot encoding of the attributes of $$s_t$$, formed by concatenating a one-hot encoding of each attribute, so that each position $$j$$ of $$x$$ represents one level of one attribute. An entry $$x_j$$ equals 1 when the individual has the attribute level represented by position $$j$$, and 0 otherwise.
