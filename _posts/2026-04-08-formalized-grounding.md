...
Other recent work has approached this question from a different layer. Curie [6] embeds rigor-enforcement modules (intra-agent validators and inter-agent control-flow state machines) directly into the agent loop, making procedural rigor a runtime concern. SMAI's contracts are doing similar work outside the agent layer rather than inside it: structural rather than procedural, enforced by the orchestrator rather than by the agent itself.
